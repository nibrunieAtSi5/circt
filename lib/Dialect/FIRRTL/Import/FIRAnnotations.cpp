...
/// references.  This function assumes that the \p target stringis a well
...
static void parseAugmentedType(
    ...
    const StringRef &companion, StringAttr name, StringAttr defName) {
  ...
  auto clazz = augmentedType.getAs<StringAttr>("class");

  // An AugmentedBundleType looks like:
  //   "defName": String
  //   "elements": Seq[AugmentedField]
  if (clazz.getValue() == (Twine(package) + ".AugmentedBundleType").str()) {
  ...
  // either be an actuall FIRRTL ground type or a GrandCentral uninferred type.
  // This can be ignored for us.
  if (clazz.getValue() == (Twine(package) + ".AugmentedGroundType").str()) {
    auto target = refTargetToString(augmentedType.getAs<DictionaryAttr>("ref"));
    NamedAttrList attr;
    attr.append("class", clazz);
  ...
  if (clazz.getValue() == (Twine(package) + ".AugmentedVectorType").str()) {
    for (auto elt : augmentedType.getAs<ArrayAttr>("elements"))
  ...
  llvm::errs() << "Handling of '" << clazz << "' is not implemented yet!\n";
...
void circt::firrtl::scatterCustomAnnotations(
  ...
  for (auto a : nonSpecificAnnotations) {
    ...
    StringAttr clazzAttr = dict.getAs<StringAttr>("class");
    ...
    // class.
    StringRef clazz = clazzAttr.getValue();
    ...
    if (clazz == "sifive.enterprise.grandcentral.DataTapsAnnotation") {
      ...
      for (auto b : dict.getAs<ArrayAttr>("keys")) {
        ...
        auto clazz = bDict.getAs<StringAttr>("class");
